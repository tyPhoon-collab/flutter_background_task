...
public class BackgroundTaskPlugin: NSObject, FlutterPlugin, CLLocationManagerDelegate {
    ...
    public func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [AnyHashable : Any] = [:]) -> Bool {
        if (launchOptions[UIApplication.LaunchOptionsKey.location] != nil) {
            ...
            let locationManager = CLLocationManager()
            locationManager.allowsBackgroundLocationUpdates = true
            let (distanceFilter, desiredAccuracy, pausesLocationUpdatesAutomatically, showsBackgroundLocationIndicator) = UserDefaultsRepository.instance.fetch()
            locationManager.showsBackgroundLocationIndicator = showsBackgroundLocationIndicator
            locationManager.pausesLocationUpdatesAutomatically = pausesLocationUpdatesAutomatically
            locationManager.distanceFilter = distanceFilter
            locationManager.desiredAccuracy = desiredAccuracy.kCLLocation
    ...
    public func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        let timeInSeconds = locations.last?.timestamp.timeIntervalSince1970
        let time = timeInSeconds.map { $0 * 1000 } // in milliseconds
        
        let location = ["lat": lat, "lng": lng, "speed": speed, "time": time] as [String : Double?]
        
        BgEventStreamHandler.eventSink?(location)
        StatusEventStreamHandler.eventSink?(
            StatusEventStreamHandler.StatusType.updated(message: "lat:\(lat ?? 0) lng:\(lng ?? 0) speed:\(speed ?? 0) time:\(time ?? 0)").value
        )
